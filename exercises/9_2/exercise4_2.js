...
const sendMarsTemperature = (contactEstablished, errorIdentified) => {  
  const currentTemperature = getMarsTemperature();
  const errorOdds = Math.random();

  if (errorOdds <= 0.6) return setTimeout(() => contactEstablished(currentTemperature), messageDelay());
  else setTimeout(() => errorIdentified('Robot is busy'), messageDelay());
}
